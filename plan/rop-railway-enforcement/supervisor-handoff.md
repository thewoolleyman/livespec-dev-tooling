...
>
> **DO NOT DRIVE THIS TRACK. DO NOT DISPATCH THE WORKER. DO NOT PICK UP A CONVERSION
> CHILD.** The hold is about **COST** — it was burning tokens and GitHub runner minutes
> the fleet is short on. ⚠️ **Nothing in this charter is retracted on the merits.**
>
> ## ⛔ THIS BANNER OVERRIDES THE DRIVE INSTRUCTIONS BELOW — READ WHY
>
> This file's §"No idle, no silent block" and §"Never end a turn without an armed
> re-entry" are written to stop a supervisor reasoning itself into standing down. **Under
> a hold they are ACTIVELY WRONG, and they appear ~200 lines ABOVE the state section that
> records the hold** — so a successor reading top-down meets "never idle, always drive"
> before it ever learns the track is stopped.
>
> **THAT IS NOT HYPOTHETICAL. IT FIRED.** On 2026-08-04 the overseer daemon marked this
> session `idle-with-context-left` and injected a nudge reading *"Do NOT stop... Pick your
> work back up and continue"* — pointing at THIS FILE, hours after the maintainer halted
> the track. **The generic never-idle pressure does not know about the hold, and it will
> hit the next reader too.** That is why this banner sits at the top rather than beside
> the state block: a correct instruction in the wrong ORDER reads as a live order.
>
> **THE CORRECT RESPONSE TO AN IDLE NUDGE WHILE THIS BANNER STANDS** is to signal
> out-of-band and stop, NOT to resume:
>
> ```sh
> echo 'blocked: rop-railway-enforcement is ON HOLD by maintainer decision' \
>   > tmp/overseer/rop-railway-enforcement-supervisor/.overseer-state
> ```
>
> ## ▶️ WHERE THE WORK WENT
>
> The track was carrying three tangled concerns and now carries one. **`8o8e` is BLOCKED
> BY `8zv3` (dependency wired in the ledger).**
>
> | concern | owner | thread |
> |---|---|---|
> | `pure_trees` role-key scope + the ROP check's decoupling from it | **`livespec-dev-tooling-8zv3`** (P1) | `plan/pure-trees-role-key-scope/` |
> | mutation-testing keystone (`livespec-mutreal.1`) | `livespec-mutreal` (P2, **`livespec`** ledger) | `plan/mutation-testing-keystone/` |
> | the ROP conversions + per-repo arming | `8o8e` — **HELD** | this thread |
>
> **A SUCCESSOR SHOULD START `pure-trees-role-key-scope` IN A NEW SESSION**, not resume
> this one. That is the successor track, and it is what unblocks this one.
>
> ⚠️ **AND THE REASON RESUMING HERE IS PARTICULARLY POINTLESS RIGHT NOW:** the ROP check
> **scans zero files in all nine repos**, so every conversion this track lands is enforced
> by NOTHING and can regress silently. **There is no ratchet until `8zv3` lands.**
>
> **TO LIFT THE HOLD:** only the maintainer lifts it. Delete this banner in the same
> change that records who lifted it and why.
...
> ⛔ **THE TRACK IS ON HOLD (2026-08-04) — see the banner at the top.** The role below describes how to supervise this track WHEN IT IS RUNNING. It is not an instruction to start it.
...
> ⛔ **SUSPENDED BY THE HOLD BANNER AT THE TOP OF THIS FILE (2026-08-04).** This section tells a supervisor never to idle. **While the hold stands that is wrong** — signal `blocked:` out-of-band and stop instead of driving.

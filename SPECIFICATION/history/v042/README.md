...
The active implementation plugin (`livespec-orchestrator-beads-fabro` per this repo's `.livespec.jsonc`) tracks work items and memos in this repo's per-repo beads Dolt tenant (tenant name `livespec-dev-tooling`). The `compat` block under the `livespec-dev-tooling` top-level key in `.livespec.jsonc` declares the supported livespec semver range and the currently-pinned livespec release tag under Livespec's cross-repo coordination contract.

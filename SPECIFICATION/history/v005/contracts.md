...
### `primary_checkout_bare_flag_set` check

Invocation: `python -m livespec_dev_tooling.checks.primary_checkout_bare_flag_set`. Exit `0` on pass OR skipped, exit `4` with structured stderr findings on fail.

The check is a port of the cross-boundary invariant declared at `livespec/SPECIFICATION/contracts.md` §"Doctor cross-boundary invariants" → §"`primary-checkout-bare-flag-set`": every livespec-governed primary checkout MUST have `core.bare = true` set in its `.git/config`. Secondary worktrees carry a standalone `.git` FILE pointing back to the bare repo and are not inspected; the check reads `core.bare` via `git config --get core.bare`, which by git's design resolves to the common config (the primary's `.git/config`) since `core.bare` is not in the worktree-config-allowed key set.
...
The check MUST NOT distinguish between absent and explicit-`false` `core.bare` values; both fail with the same corrective hint. The corrective action is `git config core.bare true` against the primary checkout (the same one-line bootstrap documented at `livespec/SPECIFICATION/non-functional-requirements.md` §"Bare-flag bootstrap procedure").
...
Consumers MAY wire the check into `just check` aggregates and/or CI matrices. Per the migration phase, each sibling's wiring is its own follow-up and MAY be staged so the wiring lands after that sibling's primary is migrated to bare.

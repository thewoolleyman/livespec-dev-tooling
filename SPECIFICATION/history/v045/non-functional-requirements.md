...
## JIT runner supervisor GitHub request discipline

The fleet JIT runner supervisor consumes a finite, installation-scoped GitHub App request budget while minting installation tokens and JIT runner configurations. This refinement does not make a rate-limit breach acceptable: a conforming supervisor MUST prevent it where possible and MUST execute the recovery behavior below if GitHub nevertheless throttles it. The controller scope MUST be the GitHub App installation/credential domain, not merely a service process. Concurrent service instances and restarts MUST use an atomic single-writer lock or equivalent shared admission primitive, so none can bypass the queue, spacing, cooldown, or retry accounting.

The controller MUST govern each installation-token mint and JIT-config mint at request-start admission. It MUST admit work through a FIFO or equivalently starvation-free, deduplicating queue with configured positive finite in-flight limit, positive minimum inter-request start-time spacing, and bounded backlog. The configuration MUST publish safe defaults and finite maxima; an invalid or unsafe setting and corrupt or unknown recovered controller state MUST fail closed before requests are emitted. When the backlog is full it MUST record an explicit overload outcome rather than bypassing admission. Startup, reconciliation, recovery, and every service-manager restart path MUST use this queue and MUST NOT independently fan out one request per desired runner slot.

For every response, the controller MUST distinguish success; primary exhaustion (`403` with `x-ratelimit-remaining: 0`); secondary throttling (`429` or GitHub secondary-limit `403`); valid `Retry-After`; valid reset-time guidance; retryable transport or transient server failure; authentication or authorization failure; and malformed or contradictory rate-limit guidance. A primary or secondary limit MUST open one shared cooldown/circuit-breaker that prevents new mint admission. When valid `Retry-After` and reset guidance both exist, the cooldown MUST honor the later boundary. When guidance is absent or malformed, it MUST use a conservative capped fallback delay and record that classification; it MUST NOT immediately retry.

Retryable work MUST use a finite configured per-work-item retry budget, capped exponential backoff, and independently sampled jitter within a documented bounded range. A shared cooldown MUST coordinate queued work so a single throttle cannot cause simultaneous retries at cooldown expiry. Authentication and authorization failures MUST be terminal and MUST NOT be retried as rate limits. Exhausting a retry budget or receiving malformed rate-limit guidance MUST produce an actionable terminal failure, retain the queued-work disposition in the record, and MUST NOT cause a supervisor or service-manager restart loop.

The controller MUST retain an append-only or equivalently queryable durable operational record with bounded retention/rollover for each admission and recovery decision: admission timestamp, queue depth, in-flight count, response classification, attempt number, selected delay and its reason, cooldown start/end, header-derived guidance with no tokens or secrets, terminal failure, queued-work disposition, and recovery/resumption. Healthy registered runners MUST remain registered and available while mint admission is delayed, and queued demand MUST remain pending rather than be discarded or satisfied by bypassing the controller. After cooldown expiry, it MAY issue only the next paced admission or probe; it MUST resume normal paced work only after that permitted operation shows recovery.

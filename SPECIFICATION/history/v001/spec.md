...
- **A `templates/library/` extraction in v1.** The library is hand-authored per livespec epic `li-fgqgnk` Phase G.2's YAGNI; if a second sibling library appears, a follow-up cycle (tracked at livespec work-item `li-huxsg3`) extracts the shared scaffold into a `templates/library/` copier template.

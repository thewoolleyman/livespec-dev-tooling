...
## Scenario: the legacy empty spelling on a union key warns and does not silently pass

Given a consumer declares a union role key as a bare `[]` or `""`

When a check consuming that key runs

Then the run MUST emit a WARN-level structured event naming both the consumer and the key

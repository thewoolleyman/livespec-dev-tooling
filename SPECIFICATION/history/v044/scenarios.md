...
## Scenario: JIT startup batch stays within the half-budget point burst

Given startup observes more runner demand than the initial batch permits

When the controller reconciles fleet demand

Then it MUST admit at most 450 of the 900 REST points per minute, or about 45 complete mint pairs

And it MUST refill further demand only from recorded budget accounting

And it MUST NOT issue one mint pair for every desired slot at startup

...
## Scenario: JIT supervisor paces a startup runner burst

Given a JIT runner supervisor starts with more queued runner slots than its configured in-flight limit

And its configured positive minimum inter-request spacing has not elapsed between two admissions

When it reconciles the desired capacity

Then it MUST admit mint requests through one bounded paced queue

And it MUST NOT issue one installation-token or JIT-config request for every desired slot at startup

And its durable operational record MUST show the queue depth, in-flight count, and every admission timestamp

## Scenario: JIT supervisor recovers from secondary limits without a retry storm

Given one JIT mint request receives either a GitHub secondary-limit `403` or `429` response with retry guidance

And additional runner demand remains queued

When the controller classifies that response

Then it MUST open one shared cooldown and prevent further mint admission until the cooldown permits it

And it MUST preserve healthy registered runners and queued demand during the cooldown

And the first post-cooldown probe or paced admission MUST precede normal recovery

And independently jittered retries MUST NOT resume as one simultaneous burst

## Scenario: JIT supervisor preserves cooldown across restart and malformed guidance

Given the JIT controller has persisted a shared cooldown after a throttle

And the supervisor restarts before that cooldown expires or receives malformed rate-limit guidance

When it recovers durable controller state

Then it MUST preserve the later valid retry boundary across restart

And it MUST use conservative fallback delay for malformed guidance rather than immediately retrying

And it MUST NOT emit a fresh startup burst

## Scenario: JIT supervisor records a terminal exhausted retry budget

Given a queued JIT mint request repeatedly receives retryable throttling responses

And its configured finite retry budget is exhausted

When the controller evaluates the next retry

Then it MUST record an actionable terminal failure with its classification and attempts

And it MUST NOT restart the supervisor or retry the request indefinitely

And unrelated healthy runners MUST remain registered

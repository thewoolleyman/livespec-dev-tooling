...
- **MINOR** — adding a new check, a new composite Action, a new reusable workflow, a new optional configuration key, or a new pin-autodiscovery format. For the detector API: adding a detector to the registry, or widening the charter-path glob set. Both may newly fail a consumer's gate on charters it already had — that is the library working, not a break, and the bump rule says MINOR so a consumer chooses when to take it.

...
| `phase2/install-node.sh` | The ONE ordered runbook for every node-local installer under `phase2/` plus `secret-reinjection/` (`sudo phase2/install-node.sh 64`): k3s config, inotify budget, AppArmor, churn-slot + timer, wedged-runner scan, log archive, the secret-reinjection unit, the converge unit + artifacts, the tmpfs mount, the storage sweep — enabled, never started. Run it on a fresh node after this script, and re-run it after any edit to that tree. |

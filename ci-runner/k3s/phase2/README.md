...
| `arc/hook-pod-template.yaml` | The pod-spec extension the ARC Kubernetes-mode container hook reads via `ACTIONS_RUNNER_CONTAINER_HOOK_TEMPLATE`. Pins the workflow pod to that profile, and carries the warm uv cache's reader side (read-only mount + `postStart` copy + `UV_CACHE_DIR`; see `warm-cache/README.md`), the warm cargo cache's reader side (the `postStart` write of `/.cargo/config.toml` pointing at `crates-proxy/`), and the fleet-wide cache kill switch (`CI_CACHE_KILL_SWITCH`). Its header records which merge semantics each field depends on and the two keys it deliberately never sets. |
...
(`crates-proxy/converge-crates-proxy.sh`) → the warm cache's

...
| `arc/hook-pod-template.yaml` | The pod-spec extension the ARC Kubernetes-mode container hook reads via `ACTIONS_RUNNER_CONTAINER_HOOK_TEMPLATE`. Pins the workflow pod to that profile, and carries the warm uv cache's reader side (read-only mount + `postStart` copy + `UV_CACHE_DIR`; see `warm-cache/README.md`), the warm cargo cache's reader side (the `postStart` write of `/.cargo/config.toml` pointing at `crates-proxy/`), the compilation cache's reader side (the `/opt/ci-runner/bin` mount, the `SCCACHE_REDIS_*` env, and the `[build] rustc-wrapper` stanza in the same file; see `sccache/README.md`), and the fleet-wide cache kill switch (`CI_CACHE_KILL_SWITCH`). Its header records which merge semantics each field depends on and the two keys it deliberately never sets. |
...
|---|---|---|---|---|
| warm cache | `ci-cache` | `/var/cache/ci-runner` | — | the warm uv cache lower (`warm-cache/`), the crates proxy's store (`crates-proxy/`), and the two tier mountpoints below |

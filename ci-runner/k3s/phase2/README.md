...
| `node-extended-resource/install-reapply-unit-exit-tests.sh` | That installer's exit tests. Runs it only through `--dry-run`, never as root, with every host-mutating tool it reaches for replaced by a tripwire and `systemctl` stubbed so BOTH branches of the stale-unit presence probe are reachable — so the suite proves the `server` sequence is byte-for-byte the one this installer ran before it knew about roles, that an `agent` invocation exits non-zero with a reason naming the server's reapply timer, that it disables and removes both units (timer first), reloads systemd and then `reset-failed`s each of them when they are present, `reset-failed`s them WITHOUT a removal or a reload when they are absent and `is-failed` still reports them failed (`livespec-dev-tooling-ssbg`), and says "nothing to remove" only when neither holds, and that the role is resolved from data and validated as data, while touching no host at all. |
...
| `install-node-exit-tests.sh` | The runbook's exit tests. Runs `install-node.sh` only through `--dry-run`, never as root, with every host-mutating tool the installers reach for replaced by a tripwire — so the suite proves that a `server` profile's plan is byte-for-byte the ordered list the single-node era ran, that an `agent` profile's plan drops exactly the eight server steps and keeps the node-local ones in the same order, that step 1's line NAMES the file that role installs (`config.yaml` plus the skip marker on a server, `config.agent.yaml` and no marker on an agent — the difference `livespec-dev-tooling-vcv4` was filed for) and, on an agent alone, the WAIT for `k3s-agent.service` and the containerd socket that a changed config earns the steps after it (`livespec-dev-tooling-4qp4`), that every skip carries a reason (the churn-slot one and the three cluster-wide sweeps' read in full, since those steps are the ones that aborted the runbook on `gmktec-xubuntu` — at 4 of 10 and then at 5b of 10), and that the profile is validated as data, while touching no host at all. Case E covers the skip's REMOVAL under a stubbed `systemctl`: the three timers `gmktec-xubuntu` was found carrying are disabled and deleted followed by one `daemon-reload` and then one `reset-failed` per unit removed (`livespec-dev-tooling-oc5g`), both halves of all four skipped unit-installing steps go timer-before-service with the same reset-failed tail, a stub reporting nothing present and nothing failed yields no removal line at all, a stub reporting nothing present but those three timers STILL FAILED — the residual state a clear over the units removed can never converge — puts the `reset-failed` lines under the `REMOVE` header with nothing disabled, unlinked or reloaded (`livespec-dev-tooling-ssbg`), a `server` yields none even when every unit is reported present, and each installer's own `SERVICE`/`TIMER` is read out of its source and required to appear in that sequence — so a unit rename in one place without the other fails here rather than leaving a renamed unit enabled and failed on the node. |

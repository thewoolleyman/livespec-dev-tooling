...
| `node-extended-resource/install-reapply-unit.sh` | Installs the patch script to `/usr/local/lib/ci-runner-k3s/` and the unit + timer to `/etc/systemd/system`, substituting the required capacity argument for the unit file's deliberate `CAPACITY_PLACEHOLDER`. Node-local, and **SERVER-only**: `--role agent` (or `CLUSTER_ROLE=agent`) REFUSES non-zero, and first disables and removes any copy an earlier run left on the node, printing each removal as a `+ ` line — the unit is ordered `Requires=k3s.service` (an agent runs `k3s-agent.service`), carries the admin `KUBECONFIG`, and reapplies a patch that selects EVERY labeled node, so installing it on an agent is wrong in KIND and not merely misconfigured (`livespec-dev-tooling-ukbp`; see `install-node.sh`'s step-4 skip). `--role` defaults to `$CLUSTER_ROLE` and then to `server`, so a bare invocation still means what it always did, and `--dry-run` prints the whole sequence and executes nothing. Installed live on `poweredge-xubuntu` at capacity 16, 2026-08-19 (`livespec-s43svm.26`) — the units had been written in `.15` but never installed, so a k3s restart would have dropped `ci-runner.io/churn-slot` and stalled ALL Kueue admission. Re-installed at capacity 64 on 2026-08-30 (the increase-ci-runners raise, livespec epic `livespec-zec4mz`); re-installed at capacity 32 on 2026-09-02 — an INTERIM throttle (maintainer decision, livespec epic `livespec-ifwnqj`; see `kueue/DERIVATION.md` "The derivation at C = 32"); re-installed at capacity 64 on 2026-09-06 once `livespec-e2vcqf` landed both NVMe drives (livespec epic `livespec-g52yrb`; "The derivation at C = 64 restored (2026-09-06)"); re-installed at capacity **32** later the same day when the soak at 64 saturated the CPU ("The step back to C = 32 on the tiered host (2026-09-06)"). The 2026-09-07 run on `gmktec-xubuntu` that established the refusal had already installed and enabled both units on that agent before dying at its own verify (`Unit k3s.service not found`), so `sudo ./install-reapply-unit.sh --role agent` on that node is what clears them. |
...
| `gate-mirror/` | R4 slice 4 of the livespec plan `k3s-on-gmktec-for-vps-usage` (epic `livespec-sab5gn`): how the tree a GATE Job runs `just check` against reaches a pod at all. Nothing has been pushed to GitHub yet — that is the point of a pre-push gate — so the driver host pushes `HEAD` into a BARE MIRROR on the `ci-cache` tier at `refs/gates/<tree-hash>` over Tailscale SSH, and a read-only in-cluster `git daemon` serves it to a pod on either node. `<tree-hash>` is `git rev-parse HEAD^{tree}`, the same value the green token is keyed on, so ONE value names the pushed tree, the served ref and the written verdict. The mirror is `/var/cache/ci-runner/gate-mirror.git` and deliberately NOT `/var/lib/git`, which belongs to the `git` package and sits on the root volume. The tailnet-ACL grant the receive path depends on is NAMED in the directory's `README.md` and owned by the `tailscale-admin` repository, which is where it is edited; nothing here carries a policy file. |
| `gate-mirror/converge-gate-mirror.sh` | The idempotent (root) converge of the gate mirror: the ci-cache tier pre-gate, the bare mirror itself (`ensure-gate-mirror.sh` — the one converge in this tree that creates HOST state that SURVIVES the boot), the sweep ConfigMap generated from `prune-gate-refs.sh`, `gate-mirror.yaml`, and a bounded rollout wait — called by the boot converge (step 8d) and by hand. `--dry-run` prints every command it would run and touches nothing: no mirror created, no cluster read, no root or kubectl precondition. |
| `gate-mirror/prune-gate-refs.sh` | The sweep, without which one ref per gated tree accumulates forever. Deletes `refs/gates/*` older than ONE DAY (aged by the loose ref file's mtime, which is the push; a hand-packed ref falls back to committer date and says so) and prunes the objects left unreachable. Run hourly by the `gate-mirror-prune` CronJob FROM THE CONFIGMAP the converge writes — refreshing the copy on disk converges nothing. The fast path is the client deleting its own ref once it has a verdict; this is the backstop for the client that dies first. |
...
(`sccache/converge-sccache-redis.sh`) → the gate mirror
(`gate-mirror/converge-gate-mirror.sh`: the bare mirror on the tier, the
read-only git daemon, the `refs/gates` sweep) → the warm cache's
...
|---|---|---|---|---|
| tier root | `ci-cache` | `/var/cache/ci-runner` | — | the crates proxy's store (`crates-proxy/`), the PyPI files proxy's store (`warm-cache/pypi-proxy/`), the sccache RDB snapshot (`sccache/`), the bare gate mirror (`gate-mirror/`, at `gate-mirror.git` — NOT `/var/lib/git`, which belongs to the `git` package and is on the root volume) and the two tier mountpoints below — the warm uv cache left it under `livespec-lvtu` |

...
| `warm-cache/` | Tier 1 of the cache tiers (livespec repo `plan/fleet-ci-runner-pool/research/design.md`), re-scoped to this lane under `livespec-s43svm.2`: a fleet-wide warm uv cache lower at `/var/lib/rancher/k3s/storage/.warm`, a hidden sibling of the runner work volumes on the `ci-workvols` tier, written only by the `ci-warm-cache` CronJob (`warm-cache-populate.sh`, generations + atomic symlink publish) and REFLINK-copied into every new work volume by the local-path provisioner's setup script (`local-path-provisioner/`; a hardlink seed from 2026-09-04 to 2026-09-06, made private per volume under `livespec-dev-tooling-hmv2bo` once the tier became XFS). `install-warm-cache.sh` derives the routed-repository list from `arc/values-*.yaml`. Measured: cold `uv sync` 7.9 s → 0.5 s warm; the hardlink seed 2.3 s and 269 MB of metadata for the 1.4 GB / 159k-file generation, the reflink seed 13.3 s and 165 MB for the 2.1 GB / 191k-file one, against 6.8 s and 2.2 GB for the per-start byte copy both replaced (`livespec-lvtu`). See `warm-cache/README.md`, whose "Lesson" records why a per-start byte copy must never ship again and whose "The hazard, closed" records why the seed is a reflink. |

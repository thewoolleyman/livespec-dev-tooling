...
|---|---|---|
| `livespec.ci_lifecycle.<class>` — one gauge per class, class name verbatim: `pvc-pending`, `bind-deadline`, `inotify-emfile`, `containerd-deadline`, `hook-failure`, `stale-listener`, `capacity-absent`, `warm-cache-oversize`, `start-seed-cost` | the count the report carries for that class. **Always emitted, 0 when clean** — an absent metric is indistinguishable from a broken emitter — with two fail-closed exceptions since `livespec-kgdlte`/`livespec-44qx`: a class whose read hit its step deadline (or was skipped), or whose judgement lacked an input — `warm-cache-oversize` with no readable budget or live generation, `start-seed-cost` with no seeded volume visible, which is every sweep on an IDLE pool — has its ZERO withheld and the reason logged; a non-zero count is sent regardless | the sweep's own findings |

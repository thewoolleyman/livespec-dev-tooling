...
| `cache-telemetry/` | The pod-side half of the same plan's telemetry clause (`livespec-dev-tooling-mlg5sf`, research/003): `ci-cache-span.sh`, the pool-provided emitter of one `cache.warm-copy` span per tier from the hook template's postStart and one `cache.job-summary` (sccache hits/misses/errors) from its preStop, posted keyless to the pod-reachable listener (port 4319) of the collector on the pod's OWN node — derived from a `status.hostIP` fieldRef rather than written as a literal address, so a second node's pods reach their own collector (the directory's README, "Why status.hostIP") — as `service.name=github-ci`; and `install-cache-telemetry.sh`, node-local like the sccache installer, placing it at `/usr/local/lib/ci-runner-k3s/bin/ci-cache-span` (mounted read-only at `/opt/ci-runner/bin`). The deterministic cold canary (`CI_CACHE_CANARY_N`) and the kill-switch tagging live in the template; the uv generation stamp in the provisioner seed. Fail-soft throughout: always exit 0, bounded by `timeout`. |

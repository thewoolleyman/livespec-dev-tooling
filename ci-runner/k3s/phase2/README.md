...
| `kueue/cluster-queue-<repo>.yaml` (10 files) | Every fleet repository's `ClusterQueue` + `LocalQueue`, one pair per repo, all in the `fleet-ci-runner-pool` cohort. Quotas derived per `DERIVATION.md` at the CURRENT capacity **C=64, restored 2026-09-06** at ten repositories (maintainer decision, livespec plan `poweredge-raid-array-maintenance` / epic `livespec-g52yrb`, once the NVMe tiering `livespec-e2vcqf` put the churn on two dedicated drives): `livespec` 9, six mid-band repos 8, `livespec-orchestrator-beads-fabro` 3, `livespec-console-beads-fabro` 2, `livespec-driver-pi` 2 — summing to exactly 64; see `DERIVATION.md` "The derivation at C = 64 restored (2026-09-06)". (History: C=32 INTERIM from 2026-09-02 to 2026-09-06 — `livespec` 5, `livespec-driver-codex` 5 then 4, five mid-band repos 4, console 1, pi 1, orchestrator 1, summing to 32, the RAID data-plane throttle of livespec epic `livespec-ifwnqj`; C=64 at nine repositories from 2026-08-30 to 2026-09-02 — `livespec` 10, `livespec-driver-codex` 9, `livespec-driver-claude` 9, four mid-band repos 8, console 2, pi 2, summing to 64, the increase-ci-runners raise `livespec-zec4mz`, to be restored when e2vcqf lands; first applied at C=16 summing to 16 across 8 repos, drift-verified 2026-08-19 `livespec-s43svm.27`; `livespec-driver-pi` joined as the ninth 2026-08-20.) |
...
summing to exactly 32; and at the current `C = 64` restored 2026-09-06 at ten
repositories it gives `livespec` 9, six mid-band repos 8,
`livespec-orchestrator-beads-fabro` 3, `livespec-console-beads-fabro` 2, and
`livespec-driver-pi` 2 — summing to exactly 64. The doubling

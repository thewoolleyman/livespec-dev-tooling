...
| `doubled repository logical ceiling` | ARC `AutoscalingRunnerSet.maxRunners` | ARC's controller — will never scale a given repo's runner pods past this cap regardless of what Kueue would otherwise admit. Since 2026-09-06 the cap is BOUNDED to `max(2 x nominalQuota, 6)` so a backlog waits on GitHub's side instead of becoming gated pods; see `kueue/DERIVATION.md` "Bounding maxRunners to the quota (2026-09-06)" | `arc/values-*.yaml` (`maxRunners`) |
...
- `maxRunners` is the formula's `doubled repository logical ceiling` — a
  per-repository ceiling ARC enforces independently of Kueue; since
  2026-09-06 it is DERIVED from the repository's quota
  (`max(2 x nominalQuota, 6)`), no longer read off its matrix width.
...
**The ceiling is also bounded from ABOVE (2026-09-06):**
`maxRunners_i = max(2 x nominalQuota_i, 6)`. A ceiling far above the quota
turns every backlog into gated pods the control plane rewrites every
reconcile — 122 of them and 11 `Slow SQL` in ten minutes at the first
fleet-wide backlog on the tiered host — while a bounded ceiling leaves the
surplus queued on GitHub at no cost. The `2 x` keeps borrowing reachable;
the floor of 6 keeps a one-slot repository from draining a lone matrix one
job at a time. Derivation and the ten values: `kueue/DERIVATION.md`
...
`patch-node-churn-capacity.sh` therefore reads capacity from the node's
own profile (`ADMISSION_CAPACITY_C`) rather than hardcoding either
number — since R5 it takes a PROFILE argument (it used to take the
capacity as a required positional) — and this is flagged as an explicit
joint-budget coordination requirement for whoever drives the incremental
cutover (`livespec-s43svm.16`) — see `VALIDATION_CHECKLIST.md` item 4. This design pass does not pick a
...
| `kueue/cluster-queue-<repo>.yaml` (10 files) | Every fleet repository's `ClusterQueue` + `LocalQueue`, one pair per repo, all in the `fleet-ci-runner-pool` cohort. Quotas derived per `DERIVATION.md` at the CURRENT capacity **C=32, re-instated 2026-09-06** at ten repositories (maintainer decision, livespec plan `poweredge-raid-array-maintenance` / epic `livespec-g52yrb`, after the soak at 64 on the tiered host saturated the CPU at 35–46 running jobs): `livespec` 5, six mid-band repos 4, `livespec-orchestrator-beads-fabro` 1, `livespec-console-beads-fabro` 1, `livespec-driver-pi` 1 — summing to exactly 32; see `DERIVATION.md` "The step back to C = 32 on the tiered host (2026-09-06)". A measured step to 40 is the recorded next candidate. (History: C=64 restored at ten repositories for about two hours on 2026-09-06 — `livespec` 9, six mid-band repos 8, `livespec-orchestrator-beads-fabro` 3, console 2, pi 2, summing to 64, "The derivation at C = 64 restored (2026-09-06)"; C=32 INTERIM from 2026-09-02 to 2026-09-06 — `livespec` 5, `livespec-driver-codex` 5 then 4, five mid-band repos 4, console 1, pi 1, orchestrator 1, summing to 32, the RAID data-plane throttle of livespec epic `livespec-ifwnqj`; C=64 at nine repositories from 2026-08-30 to 2026-09-02 — `livespec` 10, `livespec-driver-codex` 9, `livespec-driver-claude` 9, four mid-band repos 8, console 2, pi 2, summing to 64, the increase-ci-runners raise `livespec-zec4mz`, to be restored when e2vcqf lands; first applied at C=16 summing to 16 across 8 repos, drift-verified 2026-08-19 `livespec-s43svm.27`; `livespec-driver-pi` joined as the ninth 2026-08-20.) |
...
| `kueue/cluster-queue-gates.yaml` | The admission home for GATE Jobs (the `just check` aggregate offloaded to the fleet's second pool node): the `gates` `Namespace`, the `gates-flavor` `ResourceFlavor` selecting `kubernetes.io/hostname: gmktec-xubuntu` alone, the `gates-cq` `ClusterQueue` at **15 cpu / 24 Gi**, the `gate-priority` `WorkloadPriorityClass`, and the `gates-lq` `LocalQueue`. Like the phase-1 proof pair it is outside the `fleet-ci-runner-pool` cohort and quota'd on cpu/memory rather than churn-slot — deliberately, so a gate cannot spend the CI pool's admission budget — and it is excluded from the apportionment. Preemption stays at the pool's three `Never` settings; the dedicated quota, not a priority class, is what keeps gates off the CI queue. Both numbers are FIRST ESTIMATES to confirm by soak: `DERIVATION.md` "The gates quota on gmktec-xubuntu (2026-09-08)" (livespec plan `k3s-on-gmktec-for-vps-usage`, epic `livespec-sab5gn`). |
...
| `arc/values-livespec.yaml` | Worked example: livespec's per-repo `AutoscalingRunnerSet` Helm values (`maxRunners: 10` since 2026-09-06, bounded to twice its quota — 36 then 64 before; `githubConfigUrl` narrowed to this one repo, pod template wired to `livespec-lq` via the `kueue.x-k8s.io/queue-name` label and requesting one `ci-runner.io/churn-slot`). Not yet applied live — see `VALIDATION_CHECKLIST.md` item 5's disposition. |
...
| `node-extended-resource/patch-node-churn-capacity.sh` | Idempotently registers `ci-runner.io/churn-slot` as a node-status extended resource on a SINGLE named node. Takes a **PROFILE** (since R5, `livespec-dev-tooling-xa6o`), reading the node name, role and `ADMISSION_CAPACITY_C` from it and patching THAT node BY NAME — not the cluster-wide LIST it used to patch — and derives its kubeconfig from the profile's role (server admin file, or an agent's scoped node-status kubeconfig). This by-name shape is what lets an agent use the `resourceNames`-scoped node-status credential. Applied live at a small provisional capacity (4) for validation — see `VALIDATION_CHECKLIST.md` item 4. |
| `node-extended-resource/install-reapply-unit.sh` | Installs the patch script AND a copy of the node's PROFILE to `/usr/local/lib/ci-runner-k3s/` and the unit + timer to `/etc/systemd/system`, substituting the node's k3s unit (`k3s.service` on a server, `k3s-agent.service` on an agent) and the installed profile path for the unit file's deliberate `K3S_UNIT_PLACEHOLDER` and `PROFILE_PATH_PLACEHOLDER`. Takes a **PROFILE** argument (like `storage-sweep`), from which it reads `CLUSTER_ROLE`, `NODE_NAME`, `ADMISSION_CAPACITY_C` and — on an agent — `CHURN_KUBECONFIG_FILE`; the capacity is no longer a positional argument. **PER-NODE, and it installs on BOTH roles since R5** (`livespec-dev-tooling-xa6o`): `patch-node-churn-capacity.sh` now patches the SINGLE node its profile NAMES (not a cluster-wide LIST), so an agent runs its own node-local reapply timer — ordered against `k3s-agent.service`, authenticating with the scoped node-status kubeconfig `node-status-credential/` mints — rather than refusing as it did before R5. On an agent whose node-status kubeconfig is not seeded yet, the units install and enable but the immediate verify start is SKIPPED (the timer heals once `../secret-reinjection/seed-node-status-kubeconfig.sh` delivers it); a server always holds its admin kubeconfig, so its verify is unconditional. `--dry-run` prints the whole sequence and executes nothing. The server's reapply timer now patches ONLY its own node, so it no longer stamps the agent with the server's capacity. Installed live on `poweredge-xubuntu` across 2026-08 to 2026-09 at capacities 16 → 64 → 32 → 64 → 32 as the soak measured the host (see `kueue/DERIVATION.md`); R5 opens `gmktec-xubuntu` at capacity 12 by the same mechanism. |
| `node-extended-resource/install-reapply-unit-exit-tests.sh` | That installer's exit tests. Runs it only through `--dry-run`, never as root, with every host-mutating tool it reaches for replaced by a tripwire — so the suite proves that a SERVER profile and an AGENT profile each produce the right per-node install sequence (the server ordered against `k3s.service`, the agent against `k3s-agent.service`, each naming its OWN node and profile), that BOTH roles INSTALL rather than one refusing (the pre-R5 refusal is gone), that the kubectl read-back is a `get --subresource=status node` and never a bare `get node` (which would 403 on the agent under the scoped node-status credential), that `--dry-run` executes nothing, and that the profile is validated as data (a missing profile, a nonexistent path, an extra argument, an unknown option, a non-numeric `ADMISSION_CAPACITY_C`, an unknown `CLUSTER_ROLE` and an agent with an empty `CHURN_KUBECONFIG_FILE` each refused by name), while touching no host at all. |
| `node-extended-resource/patch-node-churn-capacity-exit-tests.sh` | The patch script's exit tests (added by R5). Runs it with a fake `kubectl` that records its argv and a scratch `KUBECONFIG`, never touching a host, and proves that against BOTH a server and an agent profile it exits 0 and every kubectl it issues — the readiness GET, the verify GET and the patch — carries `--subresource=status`, never a bare `get node`/`patch node`. A bare read of the parent `nodes` resource would 403 under the scoped agent node-status credential and time the readiness wait out, so this guards the fix that makes the agent reapply timer operable at all. |
...
| `host-tools/install-host-tools.sh`, `host-tools/btop-loop` | Operator-facing host tools installed into `/usr/local/bin` by `install-node.sh` step 2d. `btop-loop` runs btop in the current terminal and restarts it after an abnormal exit: btop 1.4.6 aborts with SIGABRT whenever one collect-and-draw cycle exceeds its hard-coded 5 s stall limit, which a CI fan-out with thousands of processes and hundreds of veth interfaces reliably causes (upstream aristocratos/btop issue #1746). First written straight onto poweredge-xubuntu on 2026-09-06; moved here so every host change is in gitops and rebuildable from scratch (maintainer rule, livespec plan `poweredge-raid-array-maintenance`). |
...
| `wedged-runner/install-wedged-runner-scan.sh` | Installs that scan to `/usr/local/lib/ci-runner-k3s/` and the unit + timer to `/etc/systemd/system`, substituting the required `report`/`clear` mode for the unit file's deliberate `MODE_PLACEHOLDER`. Node-local, and **SERVER-only**: `--role agent` (or `CLUSTER_ROLE=agent`) REFUSES non-zero, and first disables and removes any copy an earlier run left on the node, printing each removal as a `+ ` line — the unit is ordered `Requires=k3s.service` (an agent runs `k3s-agent.service`), carries the admin `KUBECONFIG`, and the sweep it triggers covers the pool's runner pods CLUSTER-WIDE, which the server's own timer already does for every node (`livespec-dev-tooling-qcq0`; see `install-node.sh`'s step-5 skip). Its **verify now ASSERTS**: anything other than `ActiveState=active` on the timer is fatal and names the state, where before it displayed `systemctl status` and printed DONE regardless — which is how the 2026-09-07 gmktec run reported an armed timer that had landed `failed (Result: resources)`. `--role` defaults to `$CLUSTER_ROLE` and then to `server`, and `--dry-run` prints the whole sequence and executes nothing. Installed live on `poweredge-xubuntu` in `clear` mode, 2026-08-19 (`livespec-s43svm.30`) — that script's header carries the argument for `clear` over `report` on a host with no failure routing. `sudo ./install-wedged-runner-scan.sh --role agent` on `gmktec-xubuntu` is what clears the units that run left behind. |
| `wedged-runner/install-wedged-runner-scan-exit-tests.sh` | That installer's exit tests. Cases A–D run it only through `--dry-run`, never as root, with every host-mutating tool replaced by a tripwire — proving the `server` sequence is the pre-role-awareness one plus the verify's `ActiveState` read, that an `agent` is refused with a reason naming the server's timer and both units removed (timer first), and that the role and mode are validated as data. Case E is the one that runs the installer for REAL, because the verify only runs on a real install: `install`, `systemctl`, `journalctl`, `rm` and `id` are all fakes on a scratch `PATH` and `TMPDIR` is inside the suite's scratch dir, so a timer reporting `failed` or `inactive` is proven fatal-and-named while no host is touched. |
...
| `runner-pod-lifecycle/scan-runner-pod-lifecycle.sh` | Detects the runner-pod LIFECYCLE stall — the THIRD "jobs queued, nothing starting" case — as ten named classes read from node-side observables that persist long enough for a sweep to see them: `pvc-pending`, `bind-deadline`, `inotify-emfile`, `containerd-deadline`, `hook-failure`, `stale-listener`, `capacity-absent`, `warm-cache-oversize`, `start-seed-cost`, `api-unavailable`. Every journal, log and event read is bounded to a 5-minute window (`containerd.log` rotates, and is walked backwards to the cutoff). Exits 1 naming each class with its count, 0 on a clean node, 2 when it cannot read one of its inputs — fail-closed, never a false clean. Report-only: nothing in this family is safe to auto-delete. Every sweep ends with ONE best-effort OTLP POST to the host collector — `livespec.ci_lifecycle.<class>` (one gauge per class, always emitted, 0 when clean), `livespec.ci_kueue.pending` / `.admitted`, `livespec.ci_churn_slot.allocatable` / `.quota_sum`, and, once per new populate run read from the warm cache's `last-run.json`, the `livespec.ci_warm.*` build gauges (`livespec-41w4`) — landing in the `livespec` env's `metrics` dataset; `--no-emit` skips it. See "Runner-pod lifecycle stall" → "The detector" below, and its "What every sweep emits to Honeycomb" (livespec plan `ci-runner-pod-lifecycle-reliability`, item `livespec-nhjpai`; emission `livespec-vwzv`). |
| `runner-pod-lifecycle/install-runner-pod-lifecycle-scan.sh` | Installs that scan to `/usr/local/lib/ci-runner-k3s/` and the unit + timer to `/etc/systemd/system`, enabled and started. No mode argument, by decision rather than omission: there is no clear mode (the installer's header says why). Node-local, and **SERVER-only**: `--role agent` (or `CLUSTER_ROLE=agent`) REFUSES non-zero and first disables and removes any copy an earlier run left on the node, printing each removal as a `+ ` line — the unit is ordered `Requires=k3s.service` (an agent runs `k3s-agent.service`), carries the admin `KUBECONFIG`, and reads PVCs, scheduler events, listeners and node capacity CLUSTER-WIDE, which the server's own timer already does for the whole pool (`livespec-dev-tooling-qcq0`; see `install-node.sh`'s step-5b skip). `--role` defaults to `$CLUSTER_ROLE` and then to `server`, and `--dry-run` prints the whole sequence and executes nothing. The 2026-09-07 gmktec run that established the refusal had already installed and enabled both units on that agent before dying at its own `is-active` verify, taking steps 6–10 of the runbook with it, so `sudo ./install-runner-pod-lifecycle-scan.sh --role agent` on that node is what clears them. |
| `runner-pod-lifecycle/install-runner-pod-lifecycle-scan-exit-tests.sh` | That installer's exit tests. Runs it only through `--dry-run`, never as root, with `install`, `systemctl` and `rm` replaced by tripwires — proving the `server` sequence is the pre-role-awareness one byte for byte, that an `agent` is refused with a reason naming the server's `scan-runner-pod-lifecycle.timer` and the k3s unit an agent does not run, that both units are removed (timer first) then systemd reloaded when present, `reset-failed` alone when absent but still reported failed (`livespec-dev-tooling-ssbg`), and "nothing to remove" when neither, and that the role is validated as data, while touching no host at all. |
...
| `apparmor/install-apparmor-profile.sh` | Loads that profile on a runner NODE and converges the `arc-hook-pod-template` ConfigMap. Node-local: re-run per node and after any node rebuild. `--profile-only` installs the kernel half alone, for a node that joins as an `agent`: the profile is kernel state every node needs, while the ConfigMap is one cluster object the server owns for the whole pool. `install-node.sh` passes the flag from the profile's role. |
| `arc/hook-pod-template.yaml` | The pod-spec extension the ARC Kubernetes-mode container hook reads via `ACTIONS_RUNNER_CONTAINER_HOOK_TEMPLATE`. Pins the workflow pod to that profile, and carries the warm uv cache's reader side — since `livespec-lvtu` just `UV_CACHE_DIR`, pointing uv at the seed the local-path provisioner made when the work volume was created — a reflink copy since `livespec-dev-tooling-hmv2bo`, so `UV_LINK_MODE=copy` is gone (see `warm-cache/README.md`); the cache mounts nothing from the host and copies nothing — plus the warm cargo cache's reader side (the `postStart` write of `/.cargo/config.toml` pointing at `crates-proxy/`), the compilation cache's reader side (the `/opt/ci-runner/bin` mount, the `SCCACHE_REDIS_*` env, and the `[build] rustc-wrapper` stanza in the same file; see `sccache/README.md`), and the fleet-wide cache kill switch (`CI_CACHE_KILL_SWITCH`, which also removes the job's uv seed). Its header records which merge semantics each field depends on and the two keys it deliberately never sets. |
| `arc/converge-hook-pod-template.sh` | The one idempotent converge of the `arc-hook-pod-template` ConfigMap from that file, shared by `apparmor/install-apparmor-profile.sh` and `warm-cache/install-warm-cache.sh` so the two installers cannot drift on how it is written. `--dry-run` prints the apply it would run and touches nothing. |
| `warm-cache/` | Tier 1 of the cache tiers (livespec repo `plan/fleet-ci-runner-pool/research/design.md`), re-scoped to this lane under `livespec-s43svm.2`: a fleet-wide warm uv cache lower at `/var/lib/rancher/k3s/storage/.warm`, a hidden sibling of the runner work volumes on the `ci-workvols` tier, written only by the `ci-warm-cache` CronJob (`warm-cache-populate.sh`: every generation built FROM EMPTY against the routed repositories' current `uv.lock` files through the PyPI files proxy, verified by `verify-uv-cache.py` against those locks, refused over the `warm-cache-budget` ConfigMap's fixed bytes/files budget, published by one atomic symlink rename; `livespec-41w4`) and REFLINK-copied into every new work volume by the local-path provisioner's setup script (`local-path-provisioner/`; a hardlink seed from 2026-09-04 to 2026-09-06, made private per volume under `livespec-dev-tooling-hmv2bo` once the tier became XFS). `install-warm-cache.sh` derives the routed-repository list from `arc/values-*.yaml`. Measured: cold `uv sync` 7.9 s → 0.5 s warm; the hardlink seed 2.3 s and 269 MB of metadata for the 1.4 GB / 159k-file generation the hardlink-seeded populator had grown to, the reflink seed 13.3 s and 165 MB for the 2.1 GB / 191k-file one, against 6.8 s and 2.2 GB for the per-start byte copy both replaced (`livespec-lvtu`) — the from-empty union is ~379 MB / 8k files, which is what brings the seed down. See `warm-cache/README.md`, whose "Lesson" records why a per-start byte copy must never ship again, whose "The hazard, closed" records why the seed is a reflink, and whose "Every generation is built from empty" is the bound. |
| `warm-cache/verify-uv-cache.py`, `warm-cache/uv_cache_layout.py` | The generation verifier (stdlib Python, both shipped to the populator pod in its script ConfigMap and mounted together): maps every entry of a uv 0.9.x cache — `archive-v0`, `wheels-v5`, `sdists-v9`, `git-v0`, `simple-v18` — back to a `(name, version)`, derives the one legitimate class outside every lock (build dependencies, from `[build-system].requires` closed over `Requires-Dist`), and exits 1 naming any entry no routed lock references; nothing unverified is published. `warm-cache/README.md` "Verifier" carries the layout table and why `uv cache prune`/`clean` can never do this job. |
| `warm-cache/pypi-proxy/` | The PyPI FILES proxy every from-empty rebuild fetches through: one digest-pinned nginx `proxy_cache` in front of `files.pythonhosted.org` on the `ci-cache` tier, read only by the populator (which rewrites the file-host prefix in its own clone of each lock — an index proxy caches nothing for `uv sync --frozen`), flat store so the capability-less populator can count objects for the hit ratio, verified upstream TLS, no write surface, `readOnlyRootFilesystem`. Applied by `warm-cache/converge-warm-cache.sh`. See `warm-cache/pypi-proxy/README.md`. |
| `warm-cache/registry-mirror/` | A pull-through cache of ghcr.io on the pool's LAN, so a CATTLE REBUILD — and any genuinely new image whose layers no node holds — fetches once for the whole pool instead of once per node (`livespec-h96p`). One digest-pinned `distribution` in proxy mode on the `ci-cache` tier behind `hostPort` 5001, plus `registries.yaml.template` and the role-aware `install-registry-mirror.sh` that renders it into `/etc/rancher/k3s/registries.yaml` (server: loopback; agent: the host of the profile's `CLUSTER_JOIN_ADDRESS`). It is NOT a stall fix — that premise was disproven, see `warm-cache/README.md` "Sandbox image hygiene" — and it cannot make a pull fail, because containerd's default endpoint is always tried last. Fronts ghcr.io only; every other registry is untouched. Applied 2026-09-09; its cluster half is rebuilt on every boot by `reconstruct/converge-ci-stack.sh` step 8d and copied onto the host by `reconstruct/install-converge-unit.sh` (`livespec-dev-tooling-y1t5`). |
| `warm-cache/sandbox-image-prune/` | The bound on the pool's sandbox-image set: keep the newest N releases of EACH tag family, remove older records nothing needs (`livespec-h96p`; 60 tags on 2026-09-02 became 227 on 2026-09-07, never pruned). Scoped to the `livespec-fabro-sandbox` repository as a constant, so the pinned ARC runner image is never a candidate; protects anything named by a cluster-wide PodSpec read (Jobs and CronJobs included — the warm-cache CronJob pins a tag twelve releases old and holds no pod between runs) or by any existing container; fails closed on a cluster read that errors or names nothing; re-checks immediately before removing. Removal is per-RECORD because containerd's CRI `RemoveImage` deletes every reference of the image it resolves, which is what the byte-identical-layers finding makes load-bearing. Reports by default, removes only under `--apply`. SERVER-only (the gate needs the admin kubeconfig). Its exit tests prove every one of those decisions with `crictl` and `kubectl` faked, touching no host. NOT YET INSTALLED anywhere. |
| `arc/values-livespec-overseer.yaml` | `livespec-overseer`'s per-repo `AutoscalingRunnerSet` values (`maxRunners: 8` since 2026-09-06, bounded to twice its quota — 65 before; `livespec-overseer-lq`), and the first values file wiring the hook pod template — the reference implementation the other nine copy. Applied live 2026-08-18 (Helm revision 2). |
...
| `reconstruct/converge-ci-stack.sh` | The one idempotent converge of the ENTIRE CI CLUSTER stack from this repository — ARC controller + all eleven runner scale sets + the `arc-hook-pod-template` ConfigMap + Kueue core + every `ResourceFlavor`/`ClusterQueue`/`LocalQueue` — with zero manual `kubectl`/`helm` steps. One run takes an empty k3s datastore to all listeners `Running` and Kueue admitting — after first asserting (step 1b) that THIS SERVER'S OWN node allocatable `ci-runner.io/churn-slot` equals the capacity its profile declares (the profile the INSTALLED reapply unit names in its ExecStart), and re-running `patch-node-churn-capacity.sh` for that node when it does not (`livespec-kgl3`; scoped to the server's own node by R5, so it never re-stamps the agent's per-node capacity). See "Reconstruct-on-boot" below for the scope boundary and the `install-arc.sh`/`install-kueue.sh` drift it supersedes. |
...
| `local-path-provisioner/local-path-provisioner.yaml` | The FLEET-OWNED local-path provisioner: k3s's bundled `local-storage.yaml` byte-for-byte plus the pool's tuning (`--worker-threads 8 --kube-client-qps 50 --kube-client-burst 100`, derivation in the header) and TWO seeds in its `setup` script, both `cp -a --reflink=always` inside the provisioner's Ubuntu helper pod on the volume's parent mount, both running after the runner pod is scheduled and before it starts so the runner pod's volume wait absorbs them: the WARM-CACHE seed, which reflink-copies the current warm uv cache generation (and each published warm `target/` generation) into every new work volume — 13.3 s on the 191k-file generation, measured 2026-09-06 (`livespec-lvtu`) — and the EXTERNALS seed, which reflink-copies the pinned runner image's `/home/runner/externals` from `.externals/current` into `${VOL_DIR}/externals`, marker included, so the fleet-patched container hook can skip the 595 MB copy it would otherwise make on every job start (`livespec-wm7c`; `container-hook/`). Either seed fails soft on its own — a failure leaves that seed absent and the job merely runs cold or copies as upstream — and the externals removal takes the marker before the tree, so a partial tree can never carry a marker the hook would trust. The header records the helper-pod facts both rest on, read from the provisioner's source. Applied by the converge on every boot in place of the bundled copy, which `k3s-config/` disables — the 2026-09-02 reboot proved k3s otherwise re-applies its own manifest and silently reverts the tuning (`livespec-sernfh`). |
| `k3s-config/config.yaml` + `install-k3s-config.sh` | The k3s SERVER configuration as SOURCE (`kubelet-arg: max-pods=200`; `disable: [local-storage]`; `write-kubeconfig-mode`), installed to `/etc/rancher/k3s/config.yaml` plus the `local-storage.yaml.skip` packaged-manifest marker as a second enforcement point for the disable. Replaces the hand-written config of 2026-09-01 that lived nowhere in git (`livespec-a6lxuv`'s rebuild-durability leg). `../provision-k3s.sh` runs it before the first k3s start on a server, and SKIPs it on an agent. ROLE-AWARE since `livespec-dev-tooling-vcv4`: the installer takes `--role` (passed by `install-node.sh` from the node's profile, defaulting to `server`, which is what a bare invocation always meant) and installs `config.agent.yaml` instead on an agent, writing NO marker there and removing one it finds. Takes effect on the next k3s start; never restarts k3s or k3s-agent itself — not even when it has just repaired an agent, since a restart kills every running CI job on the pool. |
| `k3s-config/config.agent.yaml` | The AGENT configuration the same installer installs to the same path on the other role: `kubelet-arg: max-pods=200` and nothing else. The two keys it deliberately OMITS are why it exists — `disable` names a packaged component only a server deploys, so it is not in k3s-agent's flag set at all, and an agent handed it exits `level=fatal ... flag provided but not defined: -disable` and restart-loops; `write-kubeconfig-mode` is the mode of an admin kubeconfig an agent never writes. Found live on `gmktec-xubuntu` 2026-09-07 (`livespec-dev-tooling-vcv4`): `install-node.sh` step 1 had installed the server file on the agent verbatim on every earlier run, and because a config is only read at the NEXT k3s start, nothing surfaced until a repair restarted the unit. Its header names each absent key; read it before adding a key to either file. |
| `k3s-config/install-k3s-config-exit-tests.sh` | That installer's exit tests. Every case resolves the installer's paths under a scratch `K3S_CONFIG_ROOT` inside its own `mktemp -d`, with a fake `id` (the installer writes `/etc/rancher/k3s` on a node, so it requires root) and `systemctl` as a TRIPWIRE — so the suite proves that `config.agent.yaml` carries neither server-only key while `config.yaml` still carries both, that a `server` run is the config-plus-marker sequence this installer always ran, that an `agent` run installs the agent file and creates not even the SERVER manifests directory, that an agent carrying the server config and the marker — the exact state `gmktec-xubuntu` was left in — is repaired with both offending keys named and nothing restarted, that the second run is a no-op that says so, and that the role is validated as data. A second set of fakes — a `systemctl is-active` that reports `activating` for a settable number of calls before `active`, and a `ctr version` that refuses the connection for a settable number of calls — then proves the agent readiness wait: a changed config on a unit still coming up waits for the unit and then for containerd, both printed and neither restarting anything; an unchanged config and a `server` run wait for nothing and probe containerd not at all; and a containerd that never answers ends the run non-zero inside its bound, naming the socket, with the config still installed. Touches no host and needs no root. |
| `storage-sweep/sweep-runner-scratch.sh` + `.service` + `install-storage-sweep.sh` | Boot-ordered (`Before=` the node's k3s unit) removal of every `pvc-*` directory under `/var/lib/rancher/k3s/storage`, gated on the datastore being EMPTY (`ConditionPathExists=!.../state.db`, re-checked by the script) so it can only ever run when no PV can reference a directory. Ends the orphan accumulation the tmpfs boot creates by design (`livespec-psq5we`: ~150 dirs / ~50 GB found 2026-09-02). ROLE-AWARE since `livespec-dev-tooling-92wn`: the installer takes the run's PROFILE (or an explicit `--role`) and resolves `CLUSTER_ROLE` out of it — a `server` gets the pre-gate and the command sequence it always had, an `agent` gets neither the pre-gate (it holds no datastore) nor `Before=k3s.service` (its k3s unit is `k3s-agent.service`). `--dry-run` prints the role-resolved ordering and the whole command sequence and executes nothing. Safe ONLY because every PVC on this pool is ephemeral runner scratch — see "Storage sweep" below before placing any other PVC on this node, and read it for what an agent's safety rests on, since the datastore gate is structurally unavailable on that role. |
| `storage-sweep/agent-ordering.conf` | The `sweep-runner-scratch.service.d/` drop-in an AGENT gets and a server does not: `Before=k3s-agent.service` and an emptied `After=`, since the server datastore mount the base unit orders after does not exist on that role. A drop-in rather than a second unit or an install-time transformation, so the base unit stays byte-identical and the role difference is DATA in a committed file — read its header for what is deliberately NOT reset. |
| `storage-sweep/install-storage-sweep-exit-tests.sh` | The sweep installer's exit tests. Runs it only through `--dry-run`, never as root, with `install` and `systemctl` replaced by tripwires — so the suite proves that a `server` plan is the pre-role-awareness command sequence and unit ordering byte for byte, that an `agent` plan applies no datastore pre-gate and is ordered against `k3s-agent.service`, that `--role` and the matching profile agree, and that a bad role, both inputs at once, a profile with no `CLUSTER_ROLE` and a missing profile are each refused by name, while touching no host at all. |
...
| `crates-proxy/converge-crates-proxy.sh` | The idempotent converge of the proxy's cluster objects with a bounded rollout wait — called by the boot converge (step 8b, before the warm cache whose populator pre-warms it) and by hand after editing the manifest. `--dry-run` prints every command it would run and touches nothing (no cluster read, no precondition check), so it is safe from a checkout on any machine. |
| `sccache/` | B1 of plan `ci-runner-cache-tiers` (`livespec-dev-tooling-ddiszt`): the shared Rust COMPILATION cache — a RAM-resident redis (digest-pinned, `maxmemory 16gb` derived against the churn-slot cap, `allkeys-lru`, RDB snapshots on the `ci-cache` tier so a host reboot restores the cache instead of leaving it cold behind the populator's busy-pool guardrail — `livespec-dev-tooling-efqeip.3`) that the pool-provided sccache binary (`install-sccache-binary.sh`, mounted read-only into every job at `/opt/ci-runner/bin`) reads from; unauthenticated = READ-ONLY by ACL, the populator the one writer with a host-held credential. The tier the console matrix's wall clock rides on. See `sccache/README.md`. |
| `sccache/converge-sccache-redis.sh` | The idempotent (root) converge of the compilation cache's cluster objects: writer credential, ACL Secret, the credential's projection into the populator's namespace, the redis Deployment + Service — called by the boot converge (step 8c) and by hand. `--dry-run` prints every command it would run and touches nothing — no credential generated, no snapshot directory created, no root or kubectl precondition — so it needs neither root nor a cluster. |
...
| `cache-telemetry/` | The pod-side half of the same plan's telemetry clause (`livespec-dev-tooling-mlg5sf`, research/003): `ci-cache-span.sh`, the pool-provided emitter of one `cache.warm-copy` span per tier from the hook template's postStart and one `cache.job-summary` (sccache hits/misses/errors) from its preStop, posted keyless to the pod-reachable listener (port 4319) of the collector on the pod's OWN node — derived IN THE POD by the emitter's `publish-endpoint` from the pod's own default gateway, which IS its node's cni0 bridge and the only address that listener binds, rather than written as a literal or read from a `status.hostIP` fieldRef that resolves to the node's LAN address, where nothing listens (the directory's README, "Why the default gateway") — as `service.name=github-ci`; and `install-cache-telemetry.sh`, node-local like the sccache installer, placing it at `/usr/local/lib/ci-runner-k3s/bin/ci-cache-span` (mounted read-only at `/opt/ci-runner/bin`). The deterministic cold canary (`CI_CACHE_CANARY_N`) and the kill-switch tagging live in the template; the uv generation stamp in the provisioner seed. Fail-soft throughout: always exit 0, bounded by `timeout`. |
| `container-hook/` | The fleet-patched ARC Kubernetes container hook (`livespec-wm7c`; livespec plan `ci-runner-pod-lifecycle-reliability`, research/005 carrier F5): the pinned runner image's hook copies `/home/runner/externals` (595 MB, 9,028 files, ~14 s) into the work volume on EVERY job start, unconditionally. `build-patched-hook.sh` derives the hook version the image bundles (v0.7.0 for runner 2.336.0), proves it byte-for-byte against the image and a reproducible unpatched rebuild, applies `externals-skip.patch` (skip the copy only when `ACTIONS_RUNNER_PRESEEDED_EXTERNALS_VERSION` names the version whose marker the seed left) and commits the bundle under `bundle/<runner-version>/` with a sha256 manifest and `BUILD-INFO`; `install-container-hook.sh` (run by `install-node.sh`, 7c) installs it to `/usr/local/lib/ci-runner-k3s/hooks/<runner-version>/` and `extract-externals.sh` puts the image's externals at `/var/lib/rancher/k3s/storage/.externals/<runner-version>/` and publishes `.externals/current` at it, the one name the provisioner's reflink seed resolves (it cannot know the runner version). The second half landed after `livespec-lvtu`: every `arc/values-*.yaml` mounts `hooks/<v>/index.js` read-only as `type: File`, points `ACTIONS_RUNNER_CONTAINER_HOOKS` at it and declares `ACTIONS_RUNNER_PRESEEDED_EXTERNALS_VERSION`, and the provisioner `setup` script seeds `${VOL_DIR}/externals`. Those three are applied together or the job fails at prepare; `runner-image.sh`'s `assert_values_pins_agree` refuses a half-rewritten values set. The skip is proposed upstream; see the directory's `README.md` for the runner-image bump procedure and the failure modes. |
...
| `warm-cache/converge-warm-cache.sh` | The idempotent converge of the warm cache's CLUSTER objects (the PyPI files proxy with a bounded rollout wait, Namespace, budget ConfigMap, CronJob, the repos ConfigMap and the script ConfigMap carrying the populator plus its verifier) with no populate Job — called by the boot converge (those objects are wiped with the tmpfs datastore; the CronJob was simply gone after the 2026-09-02 reboot) and by `install-warm-cache.sh`, which adds the attended initial populate. `--dry-run` prints every command it would run and touches no cluster; it still derives the routed-repository list, which reads only the values files beside it. |
...
| `install-node.sh` | The ONE ordered runbook for the node-local half, for BOTH node roles: runs every installer in this tree in dependency order with the right arguments (`sudo install-node.sh ../phase0-bare-metal/profiles/<node>.env`), so a from-scratch rebuild is one command. The node's `CLUSTER_ROLE` and `ADMISSION_CAPACITY_C` come from that profile, and every step is logged with the role. A `server` runs the whole list, unchanged and in the same order it always ran. An `agent` runs the node-local subset of that list, in the same order, and skips the seven server steps with a logged reason each: the iDRAC thermal convergence (PowerEdge hardware), the wedged-runner sweep (step 5), the runner-pod lifecycle sweep (step 5b) and the ARC log archive (step 6) — each carries the admin `KUBECONFIG`, the two scans are ordered `Requires=k3s.service` and cannot even start on an agent, and all three sweep or read the pool CLUSTER-WIDE, which the server's own `scan-wedged-runners.timer`, `scan-runner-pod-lifecycle.timer` and `archive-arc-logs.timer` already do for every node including this one (`livespec-dev-tooling-qcq0`) — the secret-reinjection unit and the reconstruct-on-boot converge (cluster-scoped, admin-kubeconfig-only — and the converge being the only step that applies Kueue queues and ARC scale sets, skipping it is what omits them), and the tmpfs datastore (an agent has none). AppArmor runs on both, `--profile-only` on an agent. The churn-slot reapply timer (step 4) also runs on both since R5 (`livespec-dev-tooling-xa6o`): it is PER-NODE now, so an agent installs its own node-local timer (ordered against `k3s-agent.service`, patching this node BY NAME with its profile's capacity, authenticating with the scoped node-status kubeconfig) and carries an agent NOTE for that credential's seed prerequisite. Step 1 runs on both too, and is the one step whose ARTEFACT differs: `k3s-config/config.yaml` plus the local-storage skip marker on a server, `k3s-config/config.agent.yaml` and no marker on an agent, the file named in the plan line because the server copy on an agent is fatal to k3s-agent at its next start (`livespec-dev-tooling-vcv4`). On an AGENT that step also WAITS — bounded, printed, restarting nothing — for `k3s-agent.service` to be active and for containerd to answer, but only when it actually changed the config and found the unit active or activating: on `gmktec-xubuntu` 2026-09-07 it replaced the config of a restart-looping unit and step 7c then reached `ctr images pull` five seconds before containerd was serving, so the whole runbook had to be invoked twice (`livespec-dev-tooling-4qp4`). A server has no such window and is unchanged. Enables the boot units, never starts them, never restarts k3s. An agent SKIP also REMOVES the stale server-only units an EARLIER run of this runbook installed on that node (`livespec-dev-tooling-43sc`): a skip does not invoke the installer, so the installer's own agent-side removal never ran where the units actually were, and a full exit-0 run on `gmktec-xubuntu` 2026-09-07 left server-only timers `enabled` and `failed` with `Unit k3s.service not found`. Since R5 the removable set is steps 5, 5b and 6 (the reapply timer of step 4 is no longer among them — churn-slot installs it per-node on an agent, so a reapply unit found here is legitimate and left). The units of those three steps are handed to `remove-server-only-units.sh` in ONE phase — disabled, deleted, one `daemon-reload`, then one `systemctl reset-failed` per unit removed — printed as `+ ` lines under `--dry-run` and executed live; a node carrying none of them, with none of them failed, says so and does nothing. That last pass is what makes the removal READ as done: deleting a unit file and reloading leaves the unit in `systemctl list-units --state=failed` as `not-found failed` until `reset-failed` runs or the host reboots, so the stage-4 re-run at master `49392119` removed all six files, exited 0, and still showed the same three timers failed (`livespec-dev-tooling-oc5g`). The clear is over the units NAMED and not the units this run removed, because a residual left by an EARLIER run has nothing left to remove and so would never be reached: at tree `c13617d0` the next re-run printed `nothing to remove` on that node while those same three timers were still `not-found failed` (`livespec-dev-tooling-ssbg`). Each named unit the node does not carry is therefore asked `systemctl is-failed` and cleared when the answer is `failed`. `--dry-run PROFILE` prints the plan and executes nothing; that removal's two PROBES (`systemctl list-unit-files` and `systemctl is-failed`, both reads) are the only commands a dry run reaches, so the sequence it prints is the one this host actually needs. Lists what it deliberately leaves attended (credstore seeding, the OTel collector from its own repo, the initial warm-cache populate). |
| `k3s-runtime-ready.sh` | SOURCED by `k3s-config/install-k3s-config.sh` and `container-hook/extract-externals.sh` (never run): the one place that says which socket "the containerd socket" names (`CONTAINERD_ADDRESS`, default `/run/k3s/containerd/containerd.sock`), how a script resolves containerd's `ctr` client, and what the bounded waits for that socket and for a k3s unit look like — 120 s, probed every 2 s, one progress line per 10 s, non-zero on timeout with the FATAL wording left to the caller. The probe is a `ctr version` CALL and not a stat of the socket file, because the failure it exists for is `connect: connection refused` on a socket that already exists (`livespec-dev-tooling-4qp4`). |
| `install-node-exit-tests.sh` | The runbook's exit tests. Runs `install-node.sh` only through `--dry-run`, never as root, with every host-mutating tool the installers reach for replaced by a tripwire — so the suite proves that a `server` profile's plan is byte-for-byte the ordered list it has always run, that an `agent` profile's plan drops exactly the SEVEN server steps and keeps the node-local ones (churn-slot among them since R5 — it RUNs its per-node reapply timer on an agent and carries an agent NOTE for the node-status-kubeconfig seed prerequisite) in the same order, that step 1's line NAMES the file that role installs (`config.yaml` plus the skip marker on a server, `config.agent.yaml` and no marker on an agent — the difference `livespec-dev-tooling-vcv4` was filed for) and, on an agent alone, the WAIT for `k3s-agent.service` and the containerd socket that a changed config earns the steps after it (`livespec-dev-tooling-4qp4`), that every skip carries a reason (the three cluster-wide sweeps' read in full, since those are the steps that aborted the runbook on `gmktec-xubuntu` — at 5b of 10), and that the profile is validated as data, while touching no host at all. Case E covers the skip's REMOVAL under a stubbed `systemctl`: the three server-only timers are disabled and deleted followed by one `daemon-reload` and then one `reset-failed` per unit removed (`livespec-dev-tooling-oc5g`), both halves of all THREE skipped unit-installing steps (the two scans and the ARC log archive) go timer-before-service with the same reset-failed tail, the reapply units are LEFT even when reported present (churn-slot installs them per-node on an agent, so they are legitimate), a stub reporting nothing present and nothing failed yields no removal line at all, a stub reporting nothing present but those timers STILL FAILED puts the `reset-failed` lines under the `REMOVE` header with nothing disabled, unlinked or reloaded (`livespec-dev-tooling-ssbg`), a `server` yields none even when every unit is reported present, and each server-only installer's own `SERVICE`/`TIMER` is read out of its source and required to appear in that sequence — so a unit rename in one place without the other fails here. |
| `remove-server-only-units.sh` | Disables and deletes NAMED server-only systemd units from this node, reloads systemd ONCE, and then clears the FAILED state of every named unit that has one — the ones it just removed AND the ones an EARLIER run already removed — with `systemctl reset-failed`; invoked by `install-node.sh`'s agent skip. Separate from the four installers' own removal because the caller is a SKIP: it needs a removal it can invoke without asking an installer to refuse (each refusal is a non-zero exit, right for a direct invocation and wrong for a step the plan deliberately omits) and one `daemon-reload` over the whole set rather than one per installer. Takes unit NAMES, never paths, and refuses anything that is not `NAME.service`/`NAME.timer` before joining it to `/etc/systemd/system`. Removes unit files only — never the scripts under `/usr/local/lib/ci-runner-k3s` those units exec, which each installer's server path owns. The `reset-failed` pass exists because deletion is not enough: systemd keeps a unit's failed state in the manager after its file is gone and `daemon-reload` has run, and reports it as `not-found failed` until `reset-failed` runs or the host reboots — so a removal that stopped at the reload left a permanently red failed-units read on `gmktec-xubuntu` (`livespec-dev-tooling-oc5g`). It runs AFTER the single reload, over the units actually removed and in removal order, and tolerates a non-zero exit, which is what `reset-failed` returns for a unit the manager no longer knows. And it is NOT limited to those: a unit whose file an earlier run already deleted keeps its `not-found failed` state with nothing left to remove, so a clear over the units removed in the same invocation could never converge the node it was written for — the very next re-run there printed `nothing to remove: none of the 6 server-only units named is installed on this node` while the same three timers were still listed failed (`livespec-dev-tooling-ssbg`). So each named unit this node does NOT carry is asked `systemctl is-failed` and cleared when the answer is `failed`, with no `daemon-reload` before it (nothing on the node changed) and no "nothing to remove" line beside it (something was done). `--dry-run` prints the whole sequence as `+ ` lines and executes nothing; BOTH probes are reads and run either way, so a dry run prints the sequence this host actually needs. |
| `remove-server-only-units-exit-tests.sh` | That remover's exit tests, and the ones `livespec-dev-tooling-oc5g` and `livespec-dev-tooling-ssbg` were filed for. Under a tripwire `PATH` and a stubbed `systemctl` whose presence probe, `is-failed` answer and `reset-failed` exit code are all driven from the environment: six units reported present are removed, reloaded ONCE and then `reset-failed` in removal order with every clear AFTER the reload; a named unit the probe does not report is not removed; a node carrying none of them and reporting none failed prints no command line at all and says so, for `is-failed` answering `inactive` and `unknown` alike; a named unit that is NOT installed and that `is-failed` reports `failed` draws one `reset-failed` and no disable, no `rm` and no `daemon-reload`, and no "nothing to remove" line beside it; a mixed node gets the removal, one reload, the removed unit's clear and then the residual clear; and the one case that runs the LIVE path proves a `reset-failed` exiting non-zero — a unit systemd no longer knows — is reported and does not fail the run. That live case names FIXTURE units asserted absent from `/etc/systemd/system` first, stubs `id` so the suite need not be root, and afterwards asserts no command it executed named a real unit. |

## Reconstruct-on-boot: the CI cluster stack as cattle

`reconstruct/` makes the k3s server host **reconstructible** — the
...
(`sccache/converge-sccache-redis.sh`) → the ghcr pull-through registry mirror
(`warm-cache/registry-mirror/converge-registry-mirror.sh`) → the warm cache's
cluster objects (`warm-cache/converge-warm-cache.sh`: the PyPI files proxy, then the budget ConfigMap, CronJob and script ConfigMaps, no populate Job) →
...
- **The converge asserts THIS SERVER'S OWN capacity (step 1b).** Right after
  the node reports `Ready`, before the provisioner, Kueue or any queue, it
  learns its own node and intended capacity from the PROFILE the INSTALLED
  reapply unit names in its `ExecStart` (`systemctl show
  reapply-node-extended-resource.service -p ExecStart` yields the profile
  path; `NODE_NAME` and `ADMISSION_CAPACITY_C` are read from it — the one
  already-decided number on the host, so the converge decides no number of
  its own and no second copy exists to drift; `CONVERGE_CHURN_PROFILE`
  overrides it outside the installed layout). Since R5 it is scoped to the
  SERVER's OWN node only (each node's own timer owns its capacity, so the
  server never re-stamps the agent), comparing that one node's allocatable to
  its profile capacity and, on a mismatch or absence, re-running
  `patch-node-churn-capacity.sh <profile>` (idempotent; the unit's own
  `ExecStart` target) and re-checking. The evidence line is
  `churn-slot capacity: <node> at C (M self-healed)`. A node still
...
`storage-sweep/sweep-runner-scratch.service` runs `Before=` the node's k3s unit
(`k3s.service` on a server; `k3s-agent.service` on an agent, through the
`agent-ordering.conf` drop-in the installer adds when the profile's
`CLUSTER_ROLE` says so) and
...
**On an AGENT node condition (2) has no local proof.** An agent holds no
datastore, so `state.db` never exists there: the unit's condition always
holds and the script's matching gate always passes. The agent's sweep
therefore rests on condition (1) — every PVC here is a 5 Gi ephemeral runner
work volume, and whatever was in one died with the reboot that is running the
sweep — plus the "never under a running k3s" refusal, which is why that
refusal names **both** `k3s.service` and `k3s-agent.service`: a check naming
only the server's unit would silently never fire on the one role where it is
the last gate standing (`livespec-dev-tooling-92wn`).
...
|---|---|---|---|---|
| tier root | `ci-cache` | `/var/cache/ci-runner` | — | the crates proxy's store (`crates-proxy/`), the PyPI files proxy's store (`warm-cache/pypi-proxy/`) and the two tier mountpoints below — the warm uv cache left it under `livespec-lvtu` |
...
unmounted volume lands on `/` where the volume then hides it.

Until `livespec-dev-tooling-3qyv` it did neither. It printed a NOTE telling
the operator to mount the cache tier and `mkdir` the two mountpoints by
hand and to re-run it BEFORE k3s was installed, wrote the five lines
anyway, and then died on the `findmnt --verify` that those missing
mountpoints failed — leaving `gmktec-xubuntu` on 2026-09-07 with the layout
in `/etc/fstab` and nothing mounted. Both halves of that were wrong: a step
a rehearsal cannot reproduce is a defect in the procedure
(`SPECIFICATION/non-functional-requirements.md` §"Runner-pool node rebuild
recipe"), and "before k3s" contradicted the rebuild's own stage order,
which installs k3s at stage 3 and runs this at stage 4. The installer now
does the mounting itself, at either point in that order, and on a node
whose k3s is already running it moves the store onto its tier rather than
letting a bind hide it.
...
and the negative test's case 1 re-base are cluster changes that converged
like any other (`livespec-dev-tooling-hmv2bo`, 2026-09-06).
...
2 — summing to exactly 64; at the INTERIM `C = 32` (2026-09-02 to
2026-09-06) it gave `livespec` 5, `livespec-driver-codex` 5, five mid-band
repos 4, `livespec-console-beads-fabro` 1, and `livespec-driver-pi` 1 —
summing to exactly 32; at the `C = 64` restored 2026-09-06 at ten
repositories it gave `livespec` 9, six mid-band repos 8,
`livespec-orchestrator-beads-fabro` 3, `livespec-console-beads-fabro` 2, and
`livespec-driver-pi` 2 — summing to exactly 64; and at the current `C = 32`
re-instated later that day at ten repositories it gives `livespec` 5, six
mid-band repos 4, and `livespec-orchestrator-beads-fabro`,
`livespec-console-beads-fabro` and `livespec-driver-pi` 1 each — summing to
exactly 32. The doubling
...
Since `livespec-wm7c` these files carry a `fleet-container-hook`
`hostPath` volume of `type: File` naming
...
4. Copy `arc/values-EXAMPLE-repo.yaml` to `arc/values-<repo>.yaml`. Its
   `maxRunners` is `max(2 x` the step-3 quota`, 6)` — a different
   number from the quota, larger, and BOUNDED to it; see "Two
   enforcement points, two DIFFERENT numbers" above and
   `kueue/DERIVATION.md` "Bounding maxRunners to the quota (2026-09-06)". Set
...
one timer already reaches every node, and the unit is ordered
`Requires=k3s.service` with the admin kubeconfig — neither of which an
agent has (`livespec-dev-tooling-qcq0`).
`poweredge-xubuntu` runs it in `clear` mode
...
|---|---|---|
| `pvc-pending` | Pending PVCs in `arc-runners` as JSON with `--show-managed-fields` (their `ownerReferences`, the `volume.kubernetes.io/selected-node` annotation, and the `managedFields` entry whose `fieldsV1` owns that annotation), read by python3; the ONE pod listing of the namespace (name, UID, `spec.schedulingGates`, container states — shared with `containerd-deadline`) | a claim Pending longer than 120 s (the provisioner's own helper-pod ceiling) **since its node was selected** — a claim without the `selected-node` annotation has asked the provisioner for nothing yet under `WaitForFirstConsumer` and is not counted; with it, the clock is the time of the `managedFields` entry that OWNS the annotation field, which is `kube-scheduler` upstream and the single `k3s` entry on k3s, where every in-process component writes as one manager named `k3s` (verified live on v1.36.2: every runner `-work` claim carries exactly `k3s/Update`, plus `k3s/Update/status` once Bound — the second cut matched the manager NAME against `*scheduler*` and so counted nothing); the time moves again when the provisioner strips the annotation and the scheduler re-selects (a legitimate clock restart); `creationTimestamp` is the fallback only when the owning entry's time does not parse. A PVC listing that fails for any reason withholds both this class's zero and `pvc-gated` — EXCLUDING claims OWNED (by UID, never by name: a same-name successor pod must not shadow a dead owner's claim, the `livespec-n5eudb` shape) by a pod Kueue is still holding under a scheduling gate, whose claims are Pending by design (2026-09-06: 10 gated pods' claims among 33 Pending; `livespec-kgdlte`). The work volumes are generic ephemeral volumes (`ephemeral.volumeClaimTemplate`), so `spec.volumes[*].persistentVolumeClaim.claimName` is empty on every runner pod — the first cut keyed on it and matched nothing. The excluded count is emitted as `livespec.ci_lifecycle.pvc-gated`; if the pod listing fails or hits its deadline the claims are counted, as before, and the gauge is withheld |
...
| `capacity-absent` | `k3s-role=arc-runner-host` nodes' `status.allocatable`; every `ClusterQueue`'s `nominalQuota` for `ci-runner.io/churn-slot` | a selected node without the resource OR at zero (per-node since R5 — each node carries its own capacity, poweredge 32 + gmktec 12, and a churn-slot node at 0 admits nothing); or the nodes' allocatable total below the queues' quota sum (2026-09-04: a dependency-failed boot left the reapply unit unrun and the node at none against a quota sum of 32 from 06:31Z to 07:52Z; converge assertion + timer fallback `livespec-kgl3`) |
...
**Acceptance recipe for the two F4b classes on the host** (attended;
`livespec-44qx`). `warm-cache-oversize`: patch the budget low —
`kubectl -n ci-warm-cache patch configmap warm-cache-budget -p
'{"data":{"bytes":"1"}}'` — and note that the patch ALONE does not move the
budget the sweep reads: the sweep takes `budget_*` from `last-run.json`
first and falls back to the ConfigMap only when those fields are absent, so
trigger a populate run to rewrite `last-run.json` with the new numbers
(`kubectl -n ci-warm-cache create job --from=cronjob/warm-cache-populate
budget-probe`), wait for it to finish, and the next sweep reports the class
with the bytes leg and a non-zero `livespec.ci_lifecycle.warm-cache-oversize`
row; restore the ConfigMap and run the populate again to clear it.
`start-seed-cost`: any real job start seeds a volume; the next sweep's
journal carries `livespec.ci_seed.bytes` / `.seconds` and the values reach
Honeycomb (query 5 below). An idle pool withholds the class's zero and both
seed gauges — expected, not a defect.
...
|---|---|---|
| `livespec.ci_lifecycle.<class>` — one gauge per class, class name verbatim: `pvc-pending`, `bind-deadline`, `inotify-emfile`, `containerd-deadline`, `hook-failure`, `stale-listener`, `capacity-absent`, `warm-cache-oversize`, `start-seed-cost`, `api-unavailable` | the count the report carries for that class. **Always emitted, 0 when clean** — an absent metric is indistinguishable from a broken emitter — with two fail-closed exceptions since `livespec-kgdlte`/`livespec-44qx`: a class whose read hit its step deadline (or was skipped), or whose judgement lacked an input — `warm-cache-oversize` with no readable budget or live generation, `start-seed-cost` with no seeded volume visible, which is every sweep on an IDLE pool — has its ZERO withheld and the reason logged; a non-zero count is sent regardless | the sweep's own findings |
| `livespec.ci_lifecycle.pvc-gated` | Pending PVCs owned (by UID) by a pod carrying `spec.schedulingGates` (Kueue holding it before admission), which `pvc-pending` excludes — a reading of Kueue's queue depth as the provisioner sees it, not a class. Withheld when the pod listing failed or hit its deadline, or when the PVC listing hit its deadline | the same pod and PVC listings step 1 makes |
...
| `livespec.ci_warm.generation_bytes`, `livespec.ci_warm.generation_files` | the live warm uv cache generation's size (`du -sb`) and regular-file count | `/var/lib/rancher/k3s/storage/.warm/last-run.json`, the warm-cache populator's per-run document (`warm-cache/`, `livespec-41w4`), emitted ONCE per new `run_id` (the populator runs every 30 min, the sweep every 5; the last emitted id is kept in `/var/lib/ci-runner-k3s/warm-cache-last-emitted-run`, written only after the POST succeeds). The whole family is OMITTED when the file is absent or unparseable — never a false zero |
| `livespec.ci_warm.trimmed_bytes`, `livespec.ci_warm.trimmed_files` | previous generation minus the new one, on a rebuild (0 on a verified-unchanged run; negative when the union grew) | same document |
| `livespec.ci_warm.populate_seconds` | the uv build phase's wall time (fetch, hash, from-empty sync, verify, budget, publish) | same document |
| `livespec.ci_warm.repos_synced`, `livespec.ci_warm.repos_failed` | repositories synced into the generation, and repository steps that failed (uv, cargo pre-warm, sccache) | same document |
| `livespec.ci_warm.rebuilt`, `livespec.ci_warm.refused`, `livespec.ci_warm.verified` | 0\|1 for the run: built a new generation / refused it over budget / the verifier passed | same document |
| `livespec.ci_warm.unreferenced_entries` | entries the verifier found that no routed lock references (a rejected generation) | same document |
| `livespec.ci_warm.budget_bytes`, `livespec.ci_warm.budget_files` | the budget in effect (`warm-cache-budget` ConfigMap) — the line a chart draws | same document |
...
4. **The warm-cache build gauges** (`livespec-41w4`; a separate question,
   the same dataset): one row per populate run, granularity 1800 s (the
   CronJob's cadence), filtered to rows where `livespec.ci_warm.run_epoch`
   exists — `MAX(livespec.ci_warm.generation_bytes)`,
   `MAX(livespec.ci_warm.trimmed_bytes)`,
   `MAX(livespec.ci_warm.generation_files)`,
   `MAX(livespec.ci_warm.populate_seconds)`,
   `MAX(livespec.ci_warm.proxy_hit_ratio)`, `MAX(livespec.ci_warm.refused)`,
   `MAX(livespec.ci_warm.rebuilt)`, `MAX(livespec.ci_warm.repos_failed)`,
   `MAX(livespec.ci_warm.budget_bytes)`. `generation_bytes` against
...
The warm-cache build query as a `run_query` spec:

```json
{"calculations":[{"op":"MAX","column":"livespec.ci_warm.generation_bytes"},
                 {"op":"MAX","column":"livespec.ci_warm.trimmed_bytes"},
                 {"op":"MAX","column":"livespec.ci_warm.generation_files"},
                 {"op":"MAX","column":"livespec.ci_warm.populate_seconds"},
                 {"op":"MAX","column":"livespec.ci_warm.proxy_hit_ratio"},
                 {"op":"MAX","column":"livespec.ci_warm.refused"},
                 {"op":"MAX","column":"livespec.ci_warm.rebuilt"},
                 {"op":"MAX","column":"livespec.ci_warm.repos_failed"},
                 {"op":"MAX","column":"livespec.ci_warm.budget_bytes"}],
 "filters":[{"column":"host.name","op":"=","value":"poweredge-xubuntu"},
            {"column":"livespec.ci_warm.run_epoch","op":"exists"}],
 "granularity":1800,"time_range":604800}
```
...
The refusal matters MORE here than for the two scans beside it, and for the
opposite reason: `archive-arc-logs.service` is ordered

#!/usr/bin/env bash
# install-runner-pod-lifecycle-scan-exit-tests.sh — prove the role-awareness of
# ./install-runner-pod-lifecycle-scan.sh WITHOUT touching any host: that a
...
# earlier run left on the node, and that `--dry-run` executes none of it.
#
#   A. a server plan is the PRE-CHANGE command sequence — the scan-script
#      install, the two unit installs, the reload, the enable and the
#      is-active verify — and it exits 0;
#   B. an agent invocation exits NON-ZERO, states the reason naming the SERVER's
#      lifecycle-scan timer, and, when the units are present on the node,
...
# installer reaches for (`install`, `systemctl`, `rm`): a run that executed a
# step would leave a mutating line in the tripwire file, which case C asserts it
# does not. The suite never runs as root and never needs to.
#
# Exit 0 iff every test passes. Mutates nothing outside its own scratch dir.
set -uo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCRIPT="${HERE}/install-runner-pod-lifecycle-scan.sh"
...
+ install -m 0755 scan-runner-pod-lifecycle.sh /usr/local/lib/ci-runner-k3s/scan-runner-pod-lifecycle.sh
+ install -m 0644 scan-runner-pod-lifecycle.service /etc/systemd/system/scan-runner-pod-lifecycle.service
+ install -m 0644 scan-runner-pod-lifecycle.timer /etc/systemd/system/scan-runner-pod-lifecycle.timer
+ systemctl daemon-reload
+ systemctl enable --now scan-runner-pod-lifecycle.timer
+ systemctl is-active scan-runner-pod-lifecycle.timer
+ systemctl list-timers --no-pager --all scan-runner-pod-lifecycle.timer
EOF
...
case "$AGENT_OUT" in
  *"the SERVER's own ${TIMER} already performs every five minutes"*)
    ok "the reason names the server's lifecycle-scan timer as what sweeps the pool" ;;
  *)
    no "the reason names the server's lifecycle-scan timer as what sweeps the pool" ;;
esac
case "$AGENT_OUT" in
...
# This installer takes NO positional argument — its sibling's report/clear mode
# is a decision this sweep does not have — so a stray one is a mistake rather
# than a mode, and saying so beats ignoring it.

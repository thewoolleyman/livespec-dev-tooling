#!/usr/bin/env bash
# install-reapply-unit-exit-tests.sh — prove the role-awareness of
# ./install-reapply-unit.sh WITHOUT touching any host: that a `server`
...
#
#   A. a server plan is the PRE-CHANGE command sequence, byte for byte — the
#      patch-script install, the capacity substitution, the timer install, the
#      enables, and the verify — and it exits 0;
#   B. an agent invocation exits NON-ZERO, states the reason naming the SERVER's
#      reapply timer, and, when the units are present on the node, disables and
#      removes BOTH of them (timer first) and reloads systemd; when neither is
...
#      missing capacity, a non-numeric capacity and an unknown option are each
#      handled or refused, naming what was wrong.
#
...
# installer reaches for (`install`, `systemctl`, `rm`, `kubectl`): a run that
...
mkdir -p "$FAKEBIN"
for tool in install rm kubectl; do
...
+ sed s|CAPACITY_PLACEHOLDER|32| reapply-node-extended-resource.service > /etc/systemd/system/reapply-node-extended-resource.service
...
+ kubectl get nodes -l k3s-role=arc-runner-host -o jsonpath={range .items[*]}{.metadata.name}{"\t"}{.status.allocatable.ci-runner\.io/churn-slot}{"\n"}{end}
EOF

printf '== A. server: the pre-change command sequence ==\n'
run_plan --dry-run --role server 32
...
case "$AGENT_OUT" in
  *"the server's reapply timer already performs every five minutes"*)
    ok "the reason names the server's reapply timer as what applies the patch" ;;
  *)
    no "the reason names the server's reapply timer as what applies the patch" ;;
esac
case "$AGENT_OUT" in
...
#
# The tripwire is not expected EMPTY here, unlike the sibling suites: the
# presence probe and the is-failed probe are READS, and a read is performed
# under --dry-run on purpose so the removal sequence a dry run prints is the one
# this host actually needs. What must be absent is every MUTATION.
# ---------------------------------------------------------------------------
...
REPLY_OUT="$(CLUSTER_ROLE=agent PATH="${FAKEBIN}:${PATH}" "$SCRIPT" --dry-run --role server 32 2>&1)"
REPLY_RC=$?
if [ "$REPLY_RC" -eq 0 ] && [ "$(command_lines "$REPLY_OUT")" = "$SERVER_COMMANDS" ]; then
...
refuses "an unknown --role value is refused, naming it" \
  "role must be 'server' or 'agent', got 'worker'" --dry-run --role worker 32

refuses "--role given twice is refused rather than silently ranked" \
  "--role given more than once" --dry-run --role server --role agent 32

refuses "--role with no value is refused with the usage line" \
  "--role needs a value" --dry-run --role

refuses "a server with no capacity is refused, naming what is missing" \
  "no CAPACITY given" --dry-run --role server

refuses "a non-numeric capacity is refused, naming it" \
  "CAPACITY must be a non-negative integer, got 'thirty-two'" --dry-run thirty-two

refuses "a second positional argument is refused rather than ignored" \
  "unexpected extra argument '64'" --dry-run 32 64

refuses "an unknown option is refused with the usage line" \
  "unknown option '--force'" --dry-run --force 32

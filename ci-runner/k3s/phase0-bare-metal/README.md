...
| 3 | **k3s** | `../provision-k3s.sh profiles/<node>.env` | The pinned k3s in the role this node's `CLUSTER_ROLE` declares. A `server` gets the control plane, its config installed before the first start, and the admin kubeconfig every later step reads; an `agent` gets the pinned agent join to `CLUSTER_JOIN_ADDRESS`, pinning `--node-ip` from `NODE_ADDRESS` and registering every `NODE_TAINTS` taint, with the join token read at run time out of `CLUSTER_TOKEN_FILE`. |

...
# file: `(basename, canonical body, executable)` tuples written under the
# consumer repo's `dev-tooling/` directory. The `.sh` scripts are made
# executable (the recipes invoke them directly via `./dev-tooling/…`); the
...
# NON-executable.
_PACK_PAYLOAD_FILES: tuple[tuple[str, str, bool], ...] = (
...
CANONICAL_WORKTREE_PACK_GITIGNORE_BODY = "".join(
    [
        ...
        *[f"/{name}\n" for name, _body, _executable in _PACK_PAYLOAD_FILES],
        "/.gitignore\n",
    ]
)

# The complete installed pack includes the generated ignore file last. Because
# the ignore body is derived from `_PACK_PAYLOAD_FILES`, adding a future payload
# to the installer automatically adds the matching ignore rule.
_PACK_FILES: tuple[tuple[str, str, bool], ...] = (
    *_PACK_PAYLOAD_FILES,
    (".gitignore", CANONICAL_WORKTREE_PACK_GITIGNORE_BODY, False),
)

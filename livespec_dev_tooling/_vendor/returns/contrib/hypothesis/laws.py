    ...
    def __post_init__(self) -> None:
        ...
        if self.use_init and self.container_strategy is not None:
            raise AssertionError(
                "Expected only one of `use_init` and" " `container_strategy` to be truthy"
            )
...
def type_vars_factory(thing: type[object]) -> StrategyFactory:
    ...
    return types.resolve_TypeVar(thing).filter(  # type: ignore[no-any-return]
        lambda inner: inner == inner,  # noqa: PLR0124
    )
...
def _create_law_test_case(
    ...
    *,
    ...
    test_function.__name__ = template.format(
    ...
    *,
    ...
    def factory(source: st.DataObject) -> None:
        with ExitStack() as stack:
            ...
            stack.enter_context(
                strategies_for_types(_types_to_strategies(container_type, settings))
            )
...
def _types_to_strategies(
    ...
    return settings.type_strategies | _container_mapping(container_type, settings)
